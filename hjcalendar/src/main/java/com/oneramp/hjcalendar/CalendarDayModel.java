...
public class CalendarDayModel {
    ...
    public static CalendarDayModel fromMonthNumber(int monthNumber) {
        LocalDate date = LocalDate.of(HJCalenderView.MIN_YEAR, 1, 1);
        return from(date.plusMonths(monthNumber - 1));
    }

    public static CalendarDayModel from(int day, int month, int year) {
        LocalDate date = LocalDate.of(year, month, day);
        return CalendarDayModel.from(date);
    }

    public int monthNumber() {
        if (this.getYear() >= HJCalenderView.MIN_YEAR && this.getYear() <= HJCalenderView.MAX_YEAR) {
            int yearDiff = (this.getYear() - HJCalenderView.MIN_YEAR);
